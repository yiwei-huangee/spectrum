...
def Feature_Extraction_Block(inputs, model_width, feature_number):
    ...
    latent = tf.keras.layers.Dense(feature_number, name='features',kernel_regularizer=tf.keras.regularizers.l2(0.01))(latent)
    latent = tf.keras.layers.Dense(model_width * shape[1],kernel_regularizer=tf.keras.regularizers.l2(0.01))(latent)
    ...
    def MultiResUNet(self):
        ...
        for j in range(0, self.model_depth):
            ...
            if self.LSTM == 1:
                ...
                deconv = tf.keras.layers.ConvLSTM1D(filters=np.int32(self.model_width * (2 ** (self.model_depth - j - 2))), kernel_size=3, padding='same', return_sequences=False, go_backwards=True, kernel_initializer='glorot_uniform')(merge)
            elif self.LSTM == 0:
        ...
        if self.D_S == 1:
            levels.append(tf.expand_dims(outputs,axis=-1))
            levels.reverse()
